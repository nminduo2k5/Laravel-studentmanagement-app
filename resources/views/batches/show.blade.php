  ...
  <div class="card-body">
      ...
      <div class="col-md-6">
        <div class="card h-100">
          ...
          <div class="card-body">
            ...
            <p><strong><i class="bi bi-calendar-date me-2"></i>Ngày bắt đầu:</strong> {{ \Carbon\Carbon::parse($batches->start_date)->format('d/m/Y') }}</p>
            <p><strong><i class="bi bi-people me-2"></i>Số sinh viên:</strong> {{ $batches->enrollments->count() }}</p>
            ...
            <p><strong><i class="bi bi-cash-coin me-2"></i>Tổng học phí:</strong> {{ number_format($totalFees, 0, ',', '.') }} đ</p>
            <p><strong><i class="bi bi-credit-card me-2"></i>Đã thanh toán:</strong> {{ number_format($totalPaid, 0, ',', '.') }} đ ({{ number_format($percentPaid, 1) }}%)</p>
      ...
      <div class="col-md-6">
        <div class="card h-100">
          ...
          <div class="card-body">
            ...
            <p><strong><i class="bi bi-book me-2"></i>Nội dung:</strong> {{ $batches->course->syllabus }}</p>
            <p><strong><i class="bi bi-clock me-2"></i>Thời lượng:</strong> {{ $batches->course->duration() }}</p>
            
            @if($batches->course->teachers && $batches->course->teachers->count() > 0)
              <p><strong><i class="bi bi-person-workspace me-2"></i>Giáo viên:</strong></p>
              <ul class="list-group">
                @foreach($batches->course->teachers as $teacher)
                  <li class="list-group-item d-flex justify-content-between align-items-center">
                    {{ $teacher->name }}
                    @if($teacher->pivot && $teacher->pivot->is_primary)
                      <span class="badge bg-primary rounded-pill">Giáo viên chính</span>
                    @endif
                  </li>
                @endforeach
              </ul>
            @else
              <p><strong><i class="bi bi-person-workspace me-2"></i>Giáo viên:</strong> Chưa phân công</p>
            @endif
          </div>
        </div>
      </div>
    ...
    <div class="card mb-4">
      <div class="card-header bg-success text-white">
        <h5 class="mb-0">Danh sách sinh viên đã đăng ký</h5>
      </div>
      <div class="card-body">
